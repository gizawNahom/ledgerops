...
| `POST /tenant-links`, `DELETE /tenant-links/{link_id}` (added 2026-09-07, `inter-tenant-transfer` DISTILL) | Real `chi` router over `httptest.Server`, mounted behind the existing `requireOperatorKey` group | RED scaffold today (`scaffold("authorize_tenant_pair")`/`scaffold("revoke_tenant_link")`) — same mechanism as every other driving port, no new test-side machinery |
| `POST /counterparties` (added 2026-09-07) | Real `chi` router over `httptest.Server`, `requireTenantKey`-only group | RED scaffold today (`scaffold("register_counterparty_alias")`) |
| `POST /transfers` — cross-tenant variant (added 2026-09-07) | Same route/mechanism as the existing single-tenant `POST /transfers`; `postTransferOrCrossTenantHandler` discriminates on body shape (`counterparty_alias` present) before forwarding to the unmodified existing handler or the new scaffold | RED scaffold today for the cross-tenant branch (`scaffold("send_cross_tenant_transfer")`) — the existing single-tenant branch is untouched, no new mechanism for it |
| `GET /transfers/{transfer_id}` (added 2026-09-07) | Real `chi` router over `httptest.Server`, temporarily mounted behind the existing `requireTenantKeyOrOperatorKey` middleware as a placeholder gate | RED scaffold today (`scaffold("get_transfer")`). **Known temporary gap**: the placeholder middleware is NOT the final authorization boundary — ADR-016 calls for a new, per-resource `requireTransferParty` middleware DELIVER must build before slice 05's isolation scenarios can pass; today `requireTenantKeyOrOperatorKey` grants "any authenticated tenant," which is why the slice 05 acceptance scenarios currently fail at `unidentified_caller`/wrong-refusal rather than a clean 501 in some paths (an honestly-disclosed RED classification, not a hidden gap — see `distill/wave-decisions.md`) |
...
| `TenantLinkRepository`, `CounterpartyAliasRepository`, `TransferStateRepository` (added 2026-09-07, `inter-tenant-transfer` DISTILL) | Testcontainers `postgres:16`, same fresh-container-per-package mechanism | Never faked — I11's active-uniqueness and the retry-claim's atomicity (`ClaimOne`) are both properties of the real store's constraint/locking behavior; a fake would have to reimplement the exact thing under test (brief.md § For Acceptance Designer). Not yet built — driving ports that depend on them are RED scaffolds today |
