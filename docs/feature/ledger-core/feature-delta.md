...
| OPS-12 | Multi-actor, multi-endpoint k6 load/stress/soak/success testing, `workflow_dispatch`-only, three-tier gating (informational / enforced / projected) | Amended 2026-08-31 — supersedes the single-endpoint `POST /transfers`-only scope recorded 2026-08-29. Reuses the same Prometheus + Grafana stack (no new component). Detail under § Wave: DEVOPS / [REF] Load, stress, soak, and success testing (2026-08-31) |
...
## Wave: DEVOPS / [REF] Load, stress, soak, and success testing (2026-08-31)

**Amends, does not reverse, the 2026-08-29 decision.** Trigger
(`workflow_dispatch` only), tool (k6, over vegeta, for native Prometheus
remote-write and Grafana reuse), and non-required/non-blocking posture for
`load`/`stress`/`soak` are all unchanged from § Load, stress, and soak
testing (2026-08-29) and are not re-litigated here. What changes: scope
widens from `POST /transfers` only to the full driving-port surface with a
documented actor model, the data state the workload runs against, and a new
`success` profile that is genuinely enforced rather than informational.

**Rejected simpler alternatives** (Principle 4, in addition to the two
already recorded 2026-08-29 — "no load testing, rely on race-02/race-03" and
"ad hoc single-rate vegeta with no workflow"):

- *Widen account fan-out but keep `POST /transfers` as the only endpoint
  exercised.* Fixes the hardcoded-pair problem but does not answer the
  requirement this amendment exists to satisfy — a single-endpoint test
  cannot represent usage, only throughput on one path. Insufficient.
- *Add `sleep()` for think-time to the existing script without widening
  endpoint coverage or account variety.* Same defect as above from a
  different angle — pacing realism without actor or data realism.
  Insufficient.
- *Split the workload into per-persona k6 `scenarios`* (one executor for the
  integrating-developer actor, one for the platform operator, independently
  configured and independently tagged in Grafana). Considered and rejected
  in favor of the single blended, weighted-`default()` design below: the
  per-scenario split gives cleaner Grafana attribution but doubles the
  script's surface (two executors, two functions, two independent
  parameterizations) for a distinction the endpoint-mix ratios and
  differentiated think-time already encode without it. Revisit if a future
  need arises to reason about either actor's contribution in isolation.

### Actor model and endpoint mix

Traced to the two driving journeys named in the DISCUSS wave
(`docs/product/journeys/post-a-transfer.yaml` for P1, `verify-the-books.yaml`
for P2), not picked independently of them:

| Endpoint | Weight | Actor | Journey basis |
|---|---|---|---|
| `POST /transfers` | 60% | P1 — integrating developer | primary hot path, US-1 |
| `GET /accounts/{id}` | 15% | P1 | US-1's own "decision enabled: whether to tell my user the transfer succeeded" — callers check balance after posting |
| `GET /accounts/{id}/entries` | 10% | P2 — platform operator | US-5 traceability |
| `GET /health/trial-balance` or `GET /console/verdict` | 10% | P2 | US-4, full-scan verification (D9) |
| `POST /accounts` | 5% | P1 | wallet onboarding, low frequency by nature |

`GET /metrics` is excluded — a scrape target, not a user path, per the
original 2026-08-29 scoping note.
...
- **~1.5% deliberate insufficient-funds**: amount drawn from a value known to
  ...
  — **excluded from `http_req_failed`'s effective error-rate count** via
  k6's `http.setResponseCallback(http.expectedStatuses(200, 201, 422))`,
  since it is a correct, expected refusal, not a load-test failure. This is
  a blanket classification across the whole script, not scoped to only the
  insufficient-funds slice's own requests — a deliberate simplification
  accepted because 422 is otherwise never produced by this workload; a real
  regression elsewhere producing an unexpected 422 would be masked by this
  choice, which is the traded-off cost of the simpler classification.

**Account cardinality and skew.** Pool of 20 wallet accounts (4 "hot",
16 "cold"), seeded once in `setup()`. Selection is weighted 80/20 — the 4
hot accounts receive 80% of transfer traffic; the 16 cold accounts share the
rest. This replaces both the current script's single hardcoded pair and a
flatter two-tier split considered and rejected as under-representing real
skew. **This pool is disjoint from the 250-account populated-at-scale seed
data** (§ Data state, below) — live traffic and backfilled history are
deliberately not the same accounts, so a read endpoint in the mix is never
reading rows this same run just wrote as "history."

**Amount distribution.** Three fixed buckets applied per transfer: 70% small
(`$0.01`–`$10`), 25% medium (`$10`–`$500`), 5% large (`$500`–`$4,999.99`), each
drawn from a uniform distribution within its bucket. The insufficient-funds
slice above draws from outside these buckets by construction. The same
three buckets are reused, not reinvented, for the populated-at-scale seed
data below.

**Think-time**, differentiated by actor rather than uniform: P1 (automated
caller) iterations sleep 1-3s, modeling client-side retry/backoff pacing; P2
(human operator) iterations sleep 5-15s, modeling manual dashboard-refresh
cadence. This differentiation does not change with the selected profile
(`load`/`stress`/`soak`/`tier-sweep`/`success`) — operator traffic's pacing
is not something a stress or tier-sweep profile should distort.

### Data state — populated-at-scale, not clean

**The workload runs against a populated-at-scale dataset, not an empty
database.** Two reasons the prior implicit "run against whatever `setup()`
creates" assumption doesn't hold once `GET /health/trial-balance` /
`/console/verdict` enters the mix at 10% weight:

1. Balance is stored, not derived (DDD-7) — `POST /transfers` and
   `GET /accounts/{id}` are largely insensitive to total entry-table volume.
   `GET /health/trial-balance` is the opposite by design: D9 deferred
   checkpointing in favor of full-scan verification specifically because the
   cost was expected to grow with volume, and the `elapsed_ms` instrument
   plus its 2000ms guardrail exist for exactly this. Testing that endpoint
   against a near-empty table measures nothing about its real behavior.
2. D7 makes entries append-only — for this system, "empty" is a transient
   state a live ledger occupies for roughly one deployment and never returns
   to. Populated-and-growing is the representative steady state, not the
   exception.

**Seed target: 300,000 entries** (150,000 balanced two-leg transactions),
applied once via direct SQL against the running `postgres` service before
any profile runs — not through the HTTP API, and not through
`internal/app`'s domain logic. This is a stated assumption, not a validated
inflection point — no calibration sweep (e.g., checking `elapsed_ms` at
1k/10k/100k/1M) was run first. If 300k turns out to be trivially fast for
the trial-balance scan, that is itself useful information and the seed
target should be revisited upward, not silently kept.

**Seed data has genuine variety, not a fixed value repeated 150,000 times.**
Amounts are drawn from the same three-bucket distribution as live traffic
(above); timestamps are spread across roughly the last 180 days
(`now() - random() * interval '180 days'` per transaction) rather than all
clustered at seed time — verified 2026-08-31 against a live Postgres 16
instance: 69,060 distinct amounts (range $0.01–$4,998.60), 150,000 distinct
timestamps spanning ~180 days, per-account entry counts genuinely varying
(spot-checked 600 vs. 1,800-1,801). The variation in per-account entry count
comes from a deterministic modular account-pairing formula, not true random
assignment — it produces real, non-uniform per-account history (which is
what `GET /accounts/{id}/entries` needed to be tested meaningfully), but the
specific pattern (some accounts debit-only, others credit-heavy) is a
reproducible artifact of the formula, not a claim about realistic traffic
shape — that realism is what the live-traffic 80/20 skew models instead.

**Stated limitation: this seed bypasses the domain core.** The seed SQL
writes directly to `accounts`/`transactions`/`entries`, bypassing
`internal/app`'s `PostTransfer`/`Post` use cases (DDD-14) entirely. It is
therefore **not automatically protected by the same invariant-enforcing
code path production traffic goes through** — verified once, by hand,
against the current schema and current invariants (I1: entries sum to zero
per transaction; balances reconciled). If the domain core's rules change
(a new required field, different balance semantics, a new invariant), this
SQL must be **manually** kept in sync; it will not fail loudly the way a
real integration test would if the schema's shape stays compatible but the
rules underneath it change. Accepted, empirically validated (zero
imbalance, all balances reconciled, no negative balances against a
1,000,000,000-minor-unit-per-account funding buffer), not treated as a
substitute for testing the domain core itself.

**One data state, not two.** `clean` is not additionally exercised by this
workload — it adds a second full run-matrix dimension (on top of profiles ×
tiers, below) to cover a state whose only informative value is "day one,"
already covered for correctness by the `demo`/`invariant-gates` jobs.

**Not a new environment.** This is a precondition on the existing `load`
environment (`devops/environments.yaml`), not a sixth top-level environment.
`populated`'s canonical definition (`≥1 prior transaction and entry pair`,
used by slice 04/05 correctness tests and migration safety) is unchanged —
those consumers don't need a specific row count, only "not empty." The 300k
target is `load`-specific.

### Hardware-tier extrapolation (Universal Scalability Law)

**N is a hardware tier, not concurrency.** Concurrency (k6 arrival rate /
VUs) is the mechanism used to saturate whatever tier is currently under
test, so that tier's true maximum throughput gets measured — it is not the
extrapolated variable itself.

**Tiers are capped to a single standard GitHub-hosted runner**
(`ubuntu-latest`, 4 vCPU / 16GB) — no self-hosted runner and no larger paid
GitHub-hosted SKU are provisioned for this. Everything alive during a tier's
job (app, Postgres, migrate, Prometheus, Grafana, k6 itself) shares that one
runner's 4 vCPUs. ~1 vCPU / 2GB is reserved for k6 + monitoring +
Actions overhead so the load generator itself is not starved, leaving ~3
vCPU / 14GB to split between app and Postgres, co-scaled together as one
tier axis (app-vs-DB attribution is explicitly out of scope for this pass —
see § Deferred):

| Tier | App | Postgres | Combined |
|---|---|---|---|
| 1 | 0.25 vCPU / 512MB | 0.25 vCPU / 1GB | 0.5 vCPU |
| 2 | 0.5 vCPU / 1GB | 0.5 vCPU / 2GB | 1.0 vCPU |
| 3 | 1.0 vCPU / 2GB | 1.0 vCPU / 3GB | 2.0 vCPU |
| 4 (max realizable on this runner) | 1.5 vCPU / 3GB | 1.5 vCPU / 4GB | 3.0 vCPU |

This supersedes an earlier, unconstrained 1→8 vCPU/side table considered
during design and rejected once the "cap to a standard runner" constraint
was applied — that table assumed hardware this project explicitly declined
to provision.

**Per-tier measurement.** For each tier, run the full endpoint-mix workload
above (populated-at-scale) via an escalating `ramping-arrival-rate`
executor — a longer, further-escalating extension of the existing `stress`
profile's ramp — with an automated stopping rule: increase target rate in
fixed steps; stop when throughput fails to grow by more than ~5% over the
previous step, or the error rate crosses a threshold, whichever first. The
plateau is that tier's `(N, X(N), latency-at-X(N))` point. The per-stage
throughput/latency curve is read back from Prometheus (already receiving
k6's `experimental-prometheus-rw` output), windowed by the script's own
known stage boundaries — no new tooling for this step, reusing the existing
wiring.

**Before trusting any measured plateau**, check k6's own `dropped_iterations`
metric at that tier. Because k6 and the system under test share the same 4
vCPUs, a "plateau" may reflect the load generator running out of headroom
...
least-squares line fit rather than a nonlinear solve) over the 4 tier
points, implemented in `tests/load/analysis/fit_usl.py`.

**Two ways to produce the per-tier input this script needs, added
2026-08-31 once the manual version proved too much friction in practice**:

- **Manual**: trigger the `tier-sweep` profile four times (once per `tier`
  input, 1-4), read each run's plateau throughput/latency off Grafana or
  the k6 job summary by hand, hand-fill a CSV in `fit_usl.py`'s documented
  format, then run it directly — `python3 tests/load/analysis/fit_usl.py
  measurements.csv --target-rps 579`. Fully manual; kept only because it's
  useful for ad hoc single-tier debugging, not because it's the intended
  path.
- **Automated** (`usl-sweep` profile, `tests/load/analysis/
  run_usl_sweep.py`): one `workflow_dispatch` trigger sweeps all four
  tiers in sequence within a single job — resizing the running
  `app`/`postgres` containers per tier (`docker compose up -d --no-deps`;
  Postgres's data lives in a named volume, so the populated-at-scale seed
  survives every tier's container recreation, done once, not four times),
  stepping `transfers.js`'s `fixed-rate` mode through escalating rates
  (5→10→20→40→80→160→320 rps) at each tier, and deciding the plateau in
  code from k6's own `--summary-export` JSON (`metrics.http_reqs.rate`,
  `metrics.http_req_duration["p(95)"]`, `metrics.http_req_failed.value`,
  `metrics.dropped_iterations.count`, the last of which is **absent
  entirely when zero, not present as 0** — verified against a real k6
  v2.2.0 run, not assumed from documentation, since this shape has shifted
  across k6 versions) rather than a human reading a dashboard. The
  plateau rule matches what's stated above: <5% throughput growth over the
  previous rate step, or an error rate over 5% (a breakdown threshold,
  distinct from the success scenario's 1% SLO gate) triggers recording
  that tier's point and moving to the next tier. Writes the CSV
  automatically, then invokes `fit_usl.py` automatically, appending its
  output to the job summary.

**This does mean the USL fit is now reachable from a workflow**, which
narrows the "not wired into any workflow" framing this section originally
used — `fit_usl.py` itself is still a plain, standalone script with no
independent triggering of its own; `usl-sweep` is what invokes it, and nothing
about that path is a required or blocking gate. Capacity planning here is
still an occasional, manually-triggered exercise, not a recurring one — the
change is only that "occasional" no longer requires four separate manual
runs and hand-transcribed numbers to get there.

Plateau detection is a heuristic, not a certainty, in both paths — the
automated path prints every rate step's raw numbers to the job log before
declaring a plateau, specifically so a human can sanity-check that the
auto-picked point wasn't chosen too early (a transient dip) or too late (k6
itself running out of headroom, flagged separately via
`dropped_iterations`), rather than trusting the final row blindly.

**Explicit confidence caveat.** Four points fitting three parameters (λ, α,
β) leaves one degree of freedom — a thin basis for a nonlinear regression.
Any reported curve or projected tier is **directional, not a confidence-
bounded prediction**, and must be labeled as such wherever it appears in a
report to a technical evaluator.

**Supplementary, non-authoritative local-machine data point.** One optional
run may be performed on a local development machine, outside the versioned
`workflow_dispatch` suite, to sanity-check the CI-hosted curve and
optionally contribute a 5th regression point. Requirements if used:

- Documented with exact hardware specs — CPU model and core count, RAM,
  storage type (SSD/NVMe), OS/virtualization notes. If run under WSL2, that
  must be stated explicitly: WSL2's virtualized disk I/O path is directly
  relevant here because this workload is fsync-heavy (durable commits under
  row locks), and WSL2's I/O characteristics differ materially from bare
  metal or a cloud VM's local SSD.
- Marked in every report it appears in as **non-reproducible by third
  parties** and **not part of the official CI-hosted success scenario** — it
  informs, it does not replace, the runner-hosted tiers as the
  primary, reproducible, customer-facing numbers.

**Tension with OPS-1, resolved without reversing it.** OPS-1's rationale (no
hosted environment exists, nothing needs hosting until money is real) stays
correct for production. The tier sweep produces capacity-planning estimates
from a local sandbox — cgroup limits on a CI runner, not real cloud
instance types with real network/disk/replica characteristics — and USL's
α/β are specific to this workload's shape, requiring recalibration if the
real mix, skew, or data volume diverges from what's modeled here. This is
the same honesty pattern D6 already applies to synthetic data, applied here.

**Deferred, explicitly out of scope for this amendment**: app-vs-DB
attribution within a tier, and horizontal scaling (app instance count as a
second N-axis). The latter is the point at which Kubernetes (kind/k3d
locally, or a real cluster) becomes worth its complexity — k8s's native
Deployment/Service/replica model fits horizontal scaling better than
hand-rolled `docker compose --scale` plus an ad hoc load balancer, and
testing horizontal scaling on k8s would no longer diverge from a production
topology the way testing *this* single-binary/single-Postgres system on k8s
would (OPS-2 already decided "Kubernetes has nothing to orchestrate" for the
actual shipped topology; keeping this tier sweep on `docker compose` keeps
the tested topology aligned with what's actually deployed).

### Success scenario — a real, enforced SLO

Distinct from `load` (baseline), `stress` (find the breaking point), and
`soak` (find degradation over time): `success` is a fixed-load pass/fail
...
| Latency | p95 &lt; 500ms | Reused from the original (previously informational) script thresholds — not newly invented |
| Error rate | &lt; 1% (excluding the deliberate insufficient-funds/replay slices, which are expected non-2xx/replayed responses, not failures) | Same reuse |

**The 579 rps target is decoupled from what this CI job can directly
prove.** The enforced gate runs at **Tier 4** (the largest runner-realizable
tier, § above) against **Tier 4's own measured saturation throughput** —
whatever that turns out to be — not against 579 rps directly. The exact
fixed rate for the `success` profile's stage is filled in at implementation
time from Tier 4's tier-sweep measurement (`SUCCESS_TARGET_RPS`, exposed as
a workflow variable, defaulting to a placeholder `10` until that
measurement exists), mirroring the project's existing "first-pass values,
deliberately modest... no prior numeric decision exists" pattern for the
original script's VU counts.

Separately, and **not gated**: the USL fit (§ above) projects, at the stated
latency/error bar, what tier would be required to reach 579 rps. This is
reported in the job summary as a clearly labeled, low-confidence
extrapolation — "Tier ~N projected, unvalidated beyond the measured range"
— not asserted as achieved. **This is the reconciliation between a business
target picked independently of tested hardware and a runner-capacity
ceiling picked independently of that target**: both stay true and stated,
neither is silently adjusted to match the other.

Recorded as reopening OPS-1's "no SLO exists" stance — a **provisional,
load-test-only SLO**, explicitly scoped as such, not a claimed production
guarantee, in the same pattern D6 already establishes for synthetic data.

**Where it runs**: a 4th `profile` choice (`success`) in the existing
`workflow_dispatch` input, alongside `load`/`stress`/`soak`, a 5th
(`tier-sweep`) for a single manually-selected tier's escalating measurement
run, and a 6th (`usl-sweep`) that automates `tier-sweep` across all four
tiers plus the USL fit in one trigger (§ "Fitting α (contention) and β
(coherency)" above). None of the three are promoted into `ci.yml`'s required
gates — they depend on the populated-at-scale seed step and tier-specific
resource limits that don't match a standard CI runner's default shape, and
OPS-8's trunk-based "every required gate trustworthy on every commit"
contract would be broken by a gate whose result depends on runner-capacity
variability outside the team's control.
...
### OPS-12 — scope widened from single-endpoint to multi-actor workload (2026-08-31)

**Original decision** (2026-08-29): `POST /transfers` only, no other
endpoint exercised, no data-state or hardware-tier dimension, thresholds
informational only.

**Amendment**: scope widens to the full driving-port surface with a
documented two-actor model (60/15/10/10/5 endpoint mix, 80/20 account skew,
tiered amounts, idempotency-replay and insufficient-funds slices,
differentiated think-time), adds a populated-at-scale (300k entries, with
varied amounts and timestamps spread over ~180 days) data state seeded
directly via SQL outside the domain core, adds a hardware-tier axis (USL
extrapolation, capped to a standard GitHub-hosted runner, four tiers plus
one non-authoritative local-machine supplementary point), and adds a
`success` profile with a real, three-dimensional, enforced SLO
(throughput/latency/error rate).

**This is an extension, not a reversal.** Trigger (`workflow_dispatch`
only), tool (k6), and non-blocking posture for `load`/`stress`/`soak`
specifically are unchanged. `success` is the one profile that is newly and
deliberately enforced (`abortOnFail: true`) — a change in kind for that one
profile, not for the others.

**Rationale**: recorded in full under § Load, stress, soak, and success
testing (2026-08-31), above.
