...
// status/next_attempt_at write (step 02-05) -- mechanical parity with the
// real port's identical addition, same shape as fakes_test.go's own
// equivalent in internal/app.
func (r pactFakeTransferStateRepository) AdvanceAfterLegOutcome(ctx context.Context, transferID string, status string, nextAttemptAt time.Time, reason string) error {
	state, ok := r.store.transferStates[transferID]
	if !ok {
		return fmt.Errorf("pactFakeTransferStateRepository: unknown transfer %q", transferID)
	}
