...
describe("VerdictFetchError -- the operator is told immediately, and shown where else to look", () => {
  it.skip("a failed verdict fetch shows an explicit error, not a blank page", () => {
    render(<VerdictFetchError fallbackPath={FALLBACK_HEALTH_PATH} />);
    expect(screen.getByRole("alert")).toBeInTheDocument();
  });

  it.skip("the error names the fallback path explicitly, not a generic 'try again later'", () => {
    render(<VerdictFetchError fallbackPath={FALLBACK_HEALTH_PATH} />);
